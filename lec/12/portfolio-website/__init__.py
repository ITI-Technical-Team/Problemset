...
@check50.check(has_doctype)
def test_nav():
    """index.html has <nav> containing five links (Home, About, Skills, Projects, Contact)"""
...
def test_about_section():
    ...
    for sec in sections:
        ...
        if any("about" in h.get_text().lower() for h in headings):
            about_section = sec
            break
    ...
    if not about_section:
        raise check50.Failure(
            ...
            help="Ensure one of your <section> elements contains the word 'About' or an id='about'"
        )
    ...
    if not img or not img.get("src", "").strip():
        raise check50.Failure(
            "Missing <img> or missing src attribute in <img> inside About section's <div>"
        )
...
def test_skills_section():
    ...
    for sec in sections:
        headings = sec.find_all(["h1", "h2", "h3", "h4", "h5", "h6"])
        if any("skill" in h.get_text().lower() for h in headings):
            skills_section = sec
            break
            
    if not skills_section:
        raise check50.Failure("Could not identify Skills section")
...
def test_projects_section():
    ...
    for sec in sections:
        headings = sec.find_all(["h1", "h2", "h3", "h4", "h5", "h6"])
        if any("project" in h.get_text().lower() for h in headings):
            proj_section = sec
            break
            
    if not proj_section:
        raise check50.Failure("Could not identify Projects section")
...
def test_contact_section():
    ...
    for sec in sections:
        headings = sec.find_all(["h1", "h2", "h3", "h4", "h5", "h6"])
        if any("contact" in h.get_text().lower() for h in headings):
            contact_section = sec
            break
            
    if not contact_section:
        raise check50.Failure("Could not identify Contact section")
    ...
    if len(labels) < 5:
        raise check50.Failure(
            f"Found {len(labels)} <label> element(s), expected at least 5",
