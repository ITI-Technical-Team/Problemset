...
def test_compile():
    ...
    _res = _sp.run("g++ -O1 -Wall -Wextra -Werror -fsanitize=bounds -fno-sanitize-recover=bounds -D_GLIBCXX_DEBUG print-hello.cpp -o print-hello", shell=True, capture_output=True, text=True)
...
def test_function_defined():
    """custom function is defined and called in print-hello.cpp"""
    import re
    ...
    if main_match:
        ...
        if not called:
            fn_name = custom_functions[0]
            raise check50.Failure(
                f"Custom function '{fn_name}' is defined, but never called in main().",
                help=f"Make sure you call '{fn_name}(...)' inside main()."
            )
