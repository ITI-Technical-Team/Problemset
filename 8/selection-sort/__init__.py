...
def test_compile():
    ...
    _res = _sp.run("g++ -O1 -Wall -Wextra -Werror -fsanitize=bounds -fno-sanitize-recover=bounds -D_GLIBCXX_DEBUG selection-sort.cpp -o selection-sort", shell=True, capture_output=True, text=True)
...
def test_random():
    ...
    stdin_data = f"{n}\n" + " ".join(map(str, arr))
    check50.run("./selection-sort").stdin(stdin_data, prompt=False).stdout(expected, regex=False).exit(0)
