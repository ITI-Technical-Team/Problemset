...
@check50.check()
def exists():
    """insertion-sort.cpp exists"""
    check50.exists("insertion-sort.cpp")

@check50.check(exists)
def test_compile():
    """insertion-sort.cpp compiles successfully"""
    import subprocess as _sp
    _res = _sp.run("g++ -O1 -Wall -Wextra -Werror -fsanitize=bounds -fno-sanitize-recover=bounds -D_GLIBCXX_DEBUG insertion-sort.cpp -o insertion-sort", shell=True, capture_output=True, text=True)
...
def test_random():
    ...
    stdin_data = f"{n}\n" + " ".join(map(str, arr))
    check50.run("./insertion-sort").stdin(stdin_data, prompt=False).stdout(expected, regex=False).exit(0)
