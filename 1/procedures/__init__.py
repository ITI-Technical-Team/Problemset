...
def custom_block_has_body():
    ...
    if not scratch_helper.check_custom_block_has_body(blocks):
        raise check50.Failure(
            ...
            help="Attach your loop and conditional blocks directly under the 'define' block for your custom function."
        )

@check50.check(custom_block_has_body)
def has_loop():
    """project contains a working loop inside the custom block"""
    project = scratch_helper.get_project()
    blocks = scratch_helper.get_blocks(project)
    if not scratch_helper.check_active_loop_in_custom_block(blocks):
        raise check50.Failure(
            "Did not find a loop inside the custom block.",
            help="Make sure your loop (repeat, forever, or repeat until) is attached INSIDE the custom block definition (under 'define') and has blocks inside it."
        )

@check50.check(custom_block_has_body)
def has_conditional():
    """project contains a working conditional inside the custom block"""
    project = scratch_helper.get_project()
    blocks = scratch_helper.get_blocks(project)
    if not scratch_helper.check_active_conditional_in_custom_block(blocks):
        raise check50.Failure(
            "Did not find a conditional inside the custom block.",
            help="Make sure your conditional (if or if-else) is attached INSIDE the custom block definition (under 'define') and has blocks inside it."
        )
