...
def test_example1():
    ...
    check50.run("g++ smallest-element.cpp -o smallest-element && ./smallest-element").stdin("5 3\n64 25 12 22 11", prompt=False).stdout("22", regex=False).exit(0)

@check50.check(exists)
def test_example2():
    """finds k-th smallest element correctly (Example 2)"""
    check50.run("g++ smallest-element.cpp -o smallest-element && ./smallest-element").stdin("6 4\n30 10 20 50 40 60", prompt=False).stdout("40", regex=False).exit(0)

@check50.check(exists)
def test_k1():
    """handles k = 1 (absolute smallest)"""
    check50.run("g++ smallest-element.cpp -o smallest-element && ./smallest-element").stdin("4 1\n8 9 7 6", prompt=False).stdout("6", regex=False).exit(0)
